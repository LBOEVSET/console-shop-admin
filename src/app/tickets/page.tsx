    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={(e) => handleSearch(e.target.value)} placeholder="Search by title or user…"
      ...
      {isLoading ? (
        <div className="space-y-2">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-16 rounded-xl bg-gray-100 dark:bg-gray-800 animate-pulse" />)}</div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center py-20 text-gray-400">
          <Ticket size={40} className="mb-3 opacity-30" /><p className="text-sm">No tickets found</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {filtered.map((ticket: any) => (
                  <tr key={ticket.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/60 transition group">
                    <td className="px-4 py-3">
                      <Link href={`/tickets/${ticket.id}`} className="block">
                        <p className="font-medium text-gray-900 dark:text-white truncate max-w-[220px]">{ticket.title}</p>
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span className="flex items-center gap-1 text-gray-500"><MessageSquare size={13} />{ticket.messages?.length ?? 0}</span>
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-400 hidden lg:table-cell">
                      {new Date(ticket.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <Link href={`/tickets/${ticket.id}`}><ChevronRight size={16} className="text-gray-400 group-hover:text-blue-500 transition" /></Link>
                    </td>
                  </tr>
                ))}
              </tbody>
